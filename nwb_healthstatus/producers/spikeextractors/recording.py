...
class _CommonBase:
    ...
    def __init__(self):
        # TODO -- probably should be installed into some cached location, not cwd
        pt = Path.cwd() / 'ephy_testing_data'
        existant = pt.exists()
        self.dataset = dl.install(source='https://gin.g-node.org/NeuralEnsemble/ephy_testing_data', path=pt)
        if existant:
...
def Extractors(_CommonBase):
    ...
    SCENARIOS = [
            (
                ...
                dict(
                    filename=str(Path() / "blackrock" / "blackrock_2_1" / "l101210-001"),
                    seg_index=0,
                    nsx_to_load=5
                )
            ),
            ...
            (
                ...
                dict(
                    folder_path=Path() / "openephysbinary" / "v0.5.3_two_neuropixels_stream"
                                / "Record_Node_107")
            ),
            (
                ...
                "openephysbinary/v0.5.3_two_neuropixels_stream",
                dict(
                    file_path=Path() / "openephysbinary" / "v0.5.3_two_neuropixels_stream" /
...
def Sorters(_CommonBase)
    ...
    SCENARIOS = [
            ...
            #     "nix/nixio_fr.nix",
            #     dict(file_path=str(Path() / "nix" / "nixio_fr.nix"))
            # ),
            (
            ...
            (
                ...
                dict(
                    file_or_folder_path=Path() / "spykingcircus" / "spykingcircus_example0" /
                                        "recording"
                )
            ),
