...
class _CommonBase:
    ...
    def __init__(self):
        self.pt = Path(
            user_cache_dir("nwb-healthstatus", "dandi"), 'ephy_testing_data'
        )
        existant = self.pt.exists()
        self.dataset = dl.install(source='https://gin.g-node.org/NeuralEnsemble/ephy_testing_data', path=self.pt)
        if existant:
            self.dataset.update(merge=True)  # to ensure that up to date if existed before
...
class Extractors(_CommonBase):
    ...
    SCENARIOS = [
            ...
            (
                ...
                dict(
                    folder_path=Path("openephysbinary", "v0.5.3_two_neuropixels_stream", "Record_Node_107")
                ),
            ),
            (
                ...
                "openephysbinary/v0.5.3_two_neuropixels_stream",
                dict(
                    file_path=Path("openephysbinary", "v0.5.3_two_neuropixels_stream", "Record_Node_107", "experiment1", "recording1", "continuous",
                              "Neuropix-PXI-116.0", "continuous.dat"),
                    settings_file=Path("openephysbinary",
            ...
            (
                ...
                dict(
                    file_path=Path("spikeglx", "Noise4Sam_g0", "Noise4Sam_g0_imec0",
    ...
    def create(self):
        for se_class, dataset_path, se_kwargs in self.SCENARIOS:
            ...
            for k in ["filename", "file_path", "dirname", "folder_path", "settings_file"]:
                if k in se_kwargs:
...
class Sorters(_CommonBase):
    ...
    SCENARIOS = [
            ...
            (
                ...
                dict(
                    file_or_folder_path=Path("spykingcircus", "spykingcircus_example0", "recording"),
                )
            ),
    ...
    def create(self):
        for se_class, dataset_path, se_kwargs in self.SCENARIOS:
            self.dataset.get(dataset_path)
            for k in ["filename", "file_or_folder_path", "file_path", "dirname", "folder_path"]:
                if k in se_kwargs:
                    se_kwargs[k] = self.pt / se_kwargs[k]
            if "filename" in se_kwargs:
                se_kwargs["filename"] = str(se_kwargs["filename"])
